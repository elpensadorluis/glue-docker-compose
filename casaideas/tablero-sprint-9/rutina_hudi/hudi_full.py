...
# TODO: Convertir entradas desde ssmClient a DynamoDB
try:
    event = getResolvedOptions(
        sys.argv, ["env", "curatedS3BucketName", "rawS3BucketName", "hudiStorageType",],
    )
except:
    # s3://dev-534086549449-data-raw-2/parquet/easy_store/ESTORE/VENTAS/historico/
    #
    event = {
        "env": "dev",
        "curatedS3BucketName": "dev-534086549449-data-raw-2/temp/hudi",
        "rawS3BucketLoad": "dev-534086549449-data-raw-2/parquet/easy_store/ESTORE/VENTAS/historico",
        # "rawS3BucketDelta": "dev-534086549449-data-raw-2/parquet/easy_store/ESTORE/VENTAS/historico",
        "hudiStorageType": "CoW",
    }

env = "prod" if event["env"] == "prod" else "dev"
curatedS3BucketName = event["curatedS3BucketName"]
rawS3BucketLoad = event["rawS3BucketLoad"]
# rawS3BucketDelta = event["rawS3BucketDelta"]
hudiStorageType = event["hudiStorageType"]

dropColumnList = ["db", "table_name", "Op"]

logger.info("Getting list of schema.tables that have changed.")
changeTableListDyf = glueContext.create_dynamic_frame_from_options(
    connection_type="s3",
    connection_options={
        "paths": ["s3://" + rawS3BucketLoad],
        "groupFiles": "inPartition",
        "recurse": True,
    },
    format="parquet",
    format_options={},
    transformation_ctx="changeTableListDyf",
)
...
if changeTableListDyf.count() > 0:
    ...
    # # TODO: futura rutina para sincronizar tablas hudi con redshift
    # for dbName in set([d["schema_name"] for d in changeTableList]):
    #     spark.sql("CREATE DATABASE IF NOT EXISTS " + dbName)
    #     redshiftDataClient.execute_statement(
    #         ClusterIdentifier="lakehouse-redshift-cluster",
    #         Database="lakehouse_dw",
    #         DbUser="rs_admin",
    #         Sql="CREATE EXTERNAL SCHEMA IF NOT EXISTS "
    #         + dbName
    ...
    #         + region
    #         + "' IAM_ROLE '"
    #         + boto3.client("iam").get_role(RoleName="RedshiftRoleCopyUnload")[
    #             "Role"
    #         ]["Arn"]
    #         + "'",
    #     )
    ...
    for i in changeTableList:
        ...
        if hudiStorageType == "MoR":
            tableNameCatalogCheck = (
                i["table_name"] + "_ro"
            )  # Assumption is that if _ro table exists then _rt table will also exist. Hence we are checking only for _ro.
        else:
            tableNameCatalogCheck = i[
                "table_name"
            ]  # The default config in the CF template is CoW. So assumption is that if the user hasn't explicitly requested to create MoR storage type table then we will create CoW tables. Again, if the user overwrites the config with any value other than 'MoR' we will create CoW storage type tables.
        isTableExists = False
        isPrimaryKey = False
        ...
        try:
            # TODO: este parámetro debe ser configurable desde dynamodb
            table_config = json.loads('{"primaryKey": "_c0", "partitionKey": ""}')
            try:
                ...
                isPrimaryKey = False
                logger.info(
                    "Primary key not found. An append only glueparquet table will be created."
                )
            try:
            ...
            if e.response["Error"]["Code"] == "ParameterNotFound":
                ...
                logger.info(
                    ...
                    + "."
                    + tableName
                    + " not found in parameter store. Non partitioned append only table will be created."
                )
        ...
        # Transformaciones para el dataframe unix_timestamp
        # inputDf = inputDyf.toDF().withColumn(
        #     "update_ts_dms", to_timestamp(col("update_ts_dms"))
        # )
        ...
        if isPrimaryKey:
            ...
            if isTableExists:
                # TODO: manejo de los deltas de la tabla
                ...
                if outputDf.count() > 0:
                    ...
                    if isPartitionKey:
                        ...
                        outputDf.write.format("org.apache.hudi").options(
                            **combinedConf
                        ).mode("Append").save(targetPath)
                    else:
                ...
                if outputDf_deleted.count() > 0:
                    ...
                    if isPartitionKey:
                        ...
                        outputDf_deleted = outputDf_deleted.withColumn(
                            partitionKey,
                            concat(lit(partitionKey + "="), col(partitionKey)),
                        )
                        combinedConf = {
                        ...
                        outputDf_deleted.write.format("org.apache.hudi").options(
                            **combinedConf
                        ).mode("Append").save(targetPath)
                    else:
                ...
                if outputDf.count() > 0:
                    # TODO: carga inicial << voy a sacar este fragmento de código en una etl aparte ya que solo se ejecuta la primera vez que se carga la tabla
                    ...
                    if isPartitionKey:
                        ...
                        outputDf.write.format("org.apache.hudi").options(
                            **combinedConf
                        ).mode("Overwrite").save(targetPath)
                    else:
                        logger.info("Writing to unpartitioned Hudi table.")
                        combinedConf = {
                            **commonConfig,
            ...
            outputDyf = DynamicFrame.fromDF(
                inputDf.drop(*dropColumnList), glueContext, "outputDyf"
            )
