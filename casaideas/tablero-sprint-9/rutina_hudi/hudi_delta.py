...
try:
    curatedS3BucketName = ssmClient.get_parameter(
        Name="lakehouse-curated-s3-bucket-name"
    )["Parameter"]["Value"]
    rawS3BucketName = ssmClient.get_parameter(Name="lakehouse-raw-s3-bucket-name")[
        "Parameter"
    ]["Value"]
    hudiStorageType = ssmClient.get_parameter(Name="lakehouse-hudi-storage-type")[
        "Parameter"
    ]["Value"]
except:
    curatedS3BucketName = "CuratedS3Bucket"
    rawS3BucketName = "RawS3Bucket"
    hudiStorageType = "CoW"
...
if changeTableListDyf.count() > 0:
    ...
    for dbName in set([d["schema_name"] for d in changeTableList]):
        ...
        redshiftDataClient.execute_statement(
            ...
            + region
            + "' IAM_ROLE '"
            + boto3.client("iam").get_role(RoleName="LakeHouseRedshiftGlueAccessRole")[
                "Role"
            ]["Arn"]
            + "'",
        )
    ...
    for i in changeTableList:
        ...
        inputDyf = glueContext.create_dynamic_frame_from_options(
            connection_type="s3",
            connection_options={
                "paths": ["s3://" + rawS3BucketName + "/" + dbName + "/" + tableName],
                "groupFiles": "none",
                "recurse": True,
            },
            format="parquet",
