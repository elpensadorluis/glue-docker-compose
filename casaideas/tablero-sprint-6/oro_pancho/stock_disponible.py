...
spark = (
    SparkSession.builder.appName("procuidado")
    .config(
...
redshift_user = ssm.get_parameter(Name=param_user, WithDecryption=False)["Parameter"][
    "Value"
]
redshift_pass = ssm.get_parameter(Name=param_pass, WithDecryption=True)["Parameter"][
    "Value"
]
...
puh_df = (
    ...
    .option(
        "query",
        f"select * from ventas.producto_unificado_historico where date(fecha_timestamp) = '{dia}' ",
    )
    .load()
)
...
df = (
    ...
    .option(
        "query",
        f"select * from ventas.dimension_tipo_pago where date(fecha_venta_timestamp) = '{dia}' ",
    )
    .load()
)
...
df = (
    ...
    .option(
        "query",
        f"select * from ventas.fact_producto where date(fecha_venta_timestamp) = '{dia}' ",
    )
    .load()
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.unionByName(
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "DESCUENTO",
    when(col("PRODUCTO_ID").isin([2561456, 2561465, 2561852, 2561876]), 0).otherwise(
        col("DESCUENTO")
    ),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp1, DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp1.producto_id_del, "left"
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "FABRICANTE_ID", col("fabricante_id_del")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "REGLA_ID", col("regla_id_del")
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp2, (DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp2.producto_id_del), "left"
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "COSTO_VENTA_NETO",
    when(
        col("COSTO_VENTA_NETO_del").isNotNull(), col("COSTO_VENTA_NETO_del")
    ).otherwise(col("COSTO_VENTA_NETO")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp2_2, DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp2.producto_id_del, "left"
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "COSTO_VENTA_NETO",
    when(
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp, DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp.PRODUCTO_ID_del, "left"
)

DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "CANT_UNIDADES",
    when(col("PRODUCTO_ID_del").isNotNull(), 1).otherwise(col("CANT_UNIDADES")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp,
    (DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp.PRODUCTO_ID_del)
    & (
        DM_BONIFICACION_PROCUIDADO.PORCENTAJE_DESCUENTO == temp.PORCENTAJE_DESCUENTO_del
    ),
    "left",
)

DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "PORCENTAJE_DESCUENTO_ESPERADO",
    when(col("PRODUCTO_ID_del").isNotNull(), col("porcentaje_nuevo")).otherwise(
        col("PORCENTAJE_DESCUENTO_ESPERADO")
    ),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop("PRODUCTO_ID_del")
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "PORCENTAJE_DESCUENTO_ESPERADO",
    when(col("DIA_ID").cast(DateType()) == dia, col("PORCENTAJE_DESCUENTO")).otherwise(
        col("PORCENTAJE_DESCUENTO_ESPERADO")
    ),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "PORCENTAJE_DESCUENTO_ESPERADO",
    when(
        (col("DIA_ID").cast(DateType()) == dia)
        & (col("PORCENTAJE_DESCUENTO_ESPERADO") == 1),
        0,
    ).otherwise(col("PORCENTAJE_DESCUENTO_ESPERADO")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    ...
    when(
        (col("DIA_ID").cast(DateType()) == dia)
        & (col("PORCENTAJE_DESCUENTO_ESPERADO") == 99),
        100,
    ).otherwise(col("PORCENTAJE_DESCUENTO_ESPERADO")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "CUF",
    when(col("PRODUCTO_ID_del").isNotNull(), col("CUF_del")).otherwise(col("CUF")),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop("PRODUCTO_ID_del")
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    temp,
    (DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp.PRODUCTO_ID_del)
    & (DM_BONIFICACION_PROCUIDADO.PROMOCION_ID == temp.promocion_id_del),
    "left",
)

DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "PORCENTAJE_DESCUENTO_ESPERADO",
    when(col("PRODUCTO_ID_del").isNotNull(), col("DESCUENTO_del")).otherwise(
        col("PORCENTAJE_DESCUENTO_ESPERADO")
    ),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop("PRODUCTO_ID_del")
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    ...
    when(
        (col("DIA_ID").cast(DateType()) == dia) & (col("REGLA_ID") == 1),
        (
            col("CUF")
            * col("PORCENTAJE_DESCUENTO_ESPERADO")
            * 0.01
    ...
    ).otherwise(col("BONIFICACION_ESPERADA")),
)
    ...
    when(
        ...
        & (
            (
                (
                    1
                    - (col("CUF") * col("CANT_UNIDADES"))
                    / ((col("PRECIO") * col("CANT_UNIDADES")) / 1.19)
                )
                * 100
            )
            <= 25
        ...
        (
            col("CANT_UNIDADES")
            * (col("PRECIO") * col("PORCENTAJE_DESCUENTO_ESPERADO") * 0.01)
            / 1.19
        ),
    ).otherwise(col("BONIFICACION_ESPERADA")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "BONIFICACION_ESPERADA",
    when(
        (col("DIA_ID").cast(DateType()) == dia) & (col("REGLA_ID") == 3),
        (col("COSTO") * (col("PORCENTAJE_DESCUENTO_ESPERADO") * 0.01))
        * col("CANT_UNIDADES"),
    ).otherwise(col("BONIFICACION_ESPERADA")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    ...
    when(
        ...
        & (
            (
                (
                    (col("DESCUENTO") * col("CANT_UNIDADES"))
        ...
        (
            col("CANT_UNIDADES")
            * (col("CUF") * col("PORCENTAJE_DESCUENTO_ESPERADO") * 0.01)
        ),
    ).otherwise(col("BONIFICACION_ESPERADA")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "BONIFICACION_ESPERADA",
    when(
        col("PRODUCTO_ID_del").isNotNull(), col("BONIFICACION_ESPERADA_del")
    ).otherwise(col("BONIFICACION_ESPERADA")),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop("PRODUCTO_ID_del")
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop(
    "BONIFICACION_ESPERADA_del"
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "BONIFICACION_ESPERADA",
    when(col("PRODUCTO_ID_del").isNotNull(), 0).otherwise(col("BONIFICACION_ESPERADA")),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.drop("PRODUCTO_ID_del")
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.join(
    ...
    (DM_BONIFICACION_PROCUIDADO.PRODUCTO_ID == temp.producto_id_del)
    & (DM_BONIFICACION_PROCUIDADO.PROMOCION_ID == temp.promocion_id_del)
    & (DM_BONIFICACION_PROCUIDADO.CODIGO_AUTORIZACION == temp.codigo_autorizacion_del)
    & (DM_BONIFICACION_PROCUIDADO.TIPO_TRANSACCION == "N")
    & (DM_BONIFICACION_PROCUIDADO.CANT_UNIDADES < 0),
    "left",
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "TIPO_TRANSACCION",
    when(col("unidades_del").isNotNull(), "Z").otherwise(col("TIPO_TRANSACCION")),
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.filter(
    DM_BONIFICACION_PROCUIDADO.TIPO_TRANSACCION != "Z"
)
...
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_PROMOCION", DM_BONIFICACION_PROCUIDADO["ID_PROMOCION"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_PRODUCTO", DM_BONIFICACION_PROCUIDADO["ID_PRODUCTO"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_FABRICANTE", DM_BONIFICACION_PROCUIDADO["ID_FABRICANTE"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_MES", DM_BONIFICACION_PROCUIDADO["ID_MES"].cast("string")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_LOCAL", DM_BONIFICACION_PROCUIDADO["ID_LOCAL"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_VENDEDOR", DM_BONIFICACION_PROCUIDADO["ID_VENDEDOR"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "CANTIDAD_UNIDADES", DM_BONIFICACION_PROCUIDADO["CANTIDAD_UNIDADES"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "NUMERO_DOCUMENTO", DM_BONIFICACION_PROCUIDADO["NUMERO_DOCUMENTO"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "CODIGO_AUTORIZACION",
    DM_BONIFICACION_PROCUIDADO["CODIGO_AUTORIZACION"].cast("bigint"),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "RUT_CLIENTE", DM_BONIFICACION_PROCUIDADO["RUT_CLIENTE"].cast("string")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "NOMBRE_CLIENTE", DM_BONIFICACION_PROCUIDADO["NOMBRE_CLIENTE"].cast("string")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "NOMBRE_CLIENTE", DM_BONIFICACION_PROCUIDADO["NOMBRE_CLIENTE"].cast("string")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "COSTO_UNITARIO", DM_BONIFICACION_PROCUIDADO["COSTO_UNITARIO"].cast("double")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "ID_REGLA", DM_BONIFICACION_PROCUIDADO["ID_REGLA"].cast("int")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "BONIFICACION", DM_BONIFICACION_PROCUIDADO["BONIFICACION"].cast("double")
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "DESCUENTO_PORCENTAJE_ESPERADO",
    DM_BONIFICACION_PROCUIDADO["DESCUENTO_PORCENTAJE_ESPERADO"].cast("int"),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "DESCUENTO_PORCENTAJE",
    DM_BONIFICACION_PROCUIDADO["DESCUENTO_PORCENTAJE"].cast("int"),
)
DM_BONIFICACION_PROCUIDADO = DM_BONIFICACION_PROCUIDADO.withColumn(
    "BONIFICACION_ESPERADA",
    DM_BONIFICACION_PROCUIDADO["BONIFICACION_ESPERADA"].cast("double"),
)
...
DM_BONIFICACION_PROCUIDADO.write.mode("overwrite").parquet(
    "s3://esb-bi.datalake.prod/processed/models/fact_procuidado/"
)
